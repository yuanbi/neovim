...
function build_project(compile_command)
	...
	elseif vim.fn.filereadable(cmake_lists_path) == 1 then
		...
		local cmake_pam =
			'-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_FLAGS="${CMAKE_C_FLAGS} -g -gdwarf-4" -DCMAKE_CXX_FLAGS="${CMAKE_CXX_FLAGS} -g -gdwarf-4"'
